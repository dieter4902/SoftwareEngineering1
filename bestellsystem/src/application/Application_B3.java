...
//
     ...
     */
    Application_B3() {
		eric = new Customer("Eric Meyer").setId(892474) // set id, first time
		...
		anne = new Customer("Bayer, Anne").setId(643270).addContact("anne24@yahoo.de").addContact("(030) 3481-23352");

		tim = new Customer("Tim Schulz-Mueller").setId(286516).addContact("tim2346@gmx.de");

		nadine = new Customer("Nadine-Ulla Blumenfeld").setId(412396).addContact("+49 152-92454");

		khaled = new Customer().setName("Khaled Saad Mohamed Abdelalim").setId(456454).addContact("+49 1524-12948210");
    ...
    StringBuffer fmtCustomer(StringBuffer sb, Customer customer) {
		...
		int l1 = sb.length();
		sb.append(" ").append(customer.getLastName()).append(", ").append(customer.getFirstName());
		int l2 = sb.length();
		int tabs = Math.max(1, 3 - ((l2 - l1) / 10));
		sb.append("\t".repeat(tabs)).append("contacts: ");
		for (int i = 0; i < customer.contactsCount(); i++) {
			sb.append(i > 0 ? ", " : "").append(customer.getContacts()[i]);
		}
