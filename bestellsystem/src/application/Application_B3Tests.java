...
public class Application_B3Tests extends Application_B3 {
    ...
    /**
     * Public main function that creates instance of this class, runs the
     * printCustomers() method and two test methods with more detailed tests for the
	...
	private int customerTests() throws AssertionError {
		...
		if (id == 643270) { // test id for original value
			...
			throw new AssertionError("id already set (can only be set once)");
		//
		// test: 2
		...
		if (anne.getFirstName().equals("Anna")) {
			...
			throw new AssertionError("firstName not correctly changed");
		//
		// test: 3
		...
		if (anne.getName().equals("Beyer, Anna")) {
			...
			throw new AssertionError("lastName not correctly changed");
		//
		// test: 4
		if (anne.getContacts().length == 2) {
			passed++;
		} else
			throw new AssertionError("invalid number of contacts recorded");
		//
		// test: 5
		...
		if (anne.getContacts().length == 3 && anne.getContacts()[2].equals("030 928-52345")) {
			...
			throw new AssertionError("contact not correctly recorded");
		//
		// test: 6
		...
		if (tim.contactsCount() == 2 && tim.getContacts()[0].equals("tim2346@gmx.de")
			...
			throw new AssertionError("contacts not correctly recorded");
		//
		// test: 7
		...
		if (tim.contactsCount() == 0) {
			...
			throw new AssertionError("contacts not correctly deleted");
		//
		return passed;
	}
	...
	private int customerExtendedTests() throws AssertionError {
		...
		if (tim.getFirstName().equals("Tim")) {
			...
			throw new AssertionError("setFirstName(null) not ignored, changed value");
		//
		// test: 2
		...
		if (tim.getLastName().equals("Schulz-Mueller")) {
			...
			throw new AssertionError("setLastName(null) not ignored, changed value");
		//
		// test: 3
		...
		if (tim.contactsCount() == 2 && tim.getContacts()[0].equals("030 42928424")
			...
			throw new AssertionError("deleteContact( -1 ) changed contacts, not ignored");
		//
		// test: 5
		...
		if (tim.contactsCount() == 1 && tim.getContacts()[0].equals("030 42928424")) {
			...
			throw new AssertionError("deleteContact( 1 ) contact not correctly deleted");
		//
		return passed;
	}
