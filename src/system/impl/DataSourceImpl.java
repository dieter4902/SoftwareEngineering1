...
//				e -> collector.save( e ),
                e -> {
                },    // article already collected with: inventoryManager.create( a )
                limit
        );
    }
    ...
    private <T> long read(String jsonFileName,
                          ...
                          Integer... limit) {
        int lim = Math.max(limit.length > 0 ? limit[0] : Integer.MAX_VALUE, 0);
        long count = 0;
        try (
    ...
    private boolean validArticleAttrs(String id, long unitPrice, String description, Currency currency) {
        boolean valid = id != null && id.length() > 0 && !id.equalsIgnoreCase("null");
